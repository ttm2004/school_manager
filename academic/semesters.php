...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isAcademicManager()) {
        $_SESSION['_flash'] = ['type'=>'danger','message'=>'Chỉ Trưởng phòng mới có quyền quản lý học kỳ.'];
        header('Location: semesters.php'); exit();
    }
    ...
    if ($action === 'add') {
        ...
        if ($name && $schoolYear) {
            ...
            if ($stmt->execute()) {
                ...
                $_SESSION['_flash'] = ['type'=>'success','message'=>'Thêm học kỳ thành công.' . ($normalized ? ' Đã đồng bộ ' . $normalized . ' mốc thời gian liên quan.' : '')];
            } else {
                $_SESSION['_flash'] = ['type'=>'danger','message'=>'Lỗi: '.$conn->error];
            }
            $stmt->close();
        } else { $_SESSION['_flash']=['type'=>'danger','message'=>'Vui lòng nhập tên và năm học.']; }
        header('Location: semesters.php'); exit();
    }
    ...
    if ($action === 'edit') {
        ...
        if ($id && $name) {
            ...
            if ($stmt->execute()) {
                ...
                $_SESSION['_flash'] = ['type'=>'success','message'=>'Cập nhật thành công.' . ($normalized ? ' Đã đồng bộ ' . $normalized . ' mốc thời gian liên quan.' : '')];
            } else {
                $_SESSION['_flash'] = ['type'=>'danger','message'=>'Lỗi: '.$conn->error];
            }
            $stmt->close();
    ...
    if ($action === 'open_registration') {
        ...
        if ($stmt->execute()) {
            ...
            $content = "Học kỳ đã mở đăng ký học phần. Hạn đăng ký: " . date('d/m/Y H:i', strtotime($regEnd));
            ...
            $stmtN->close();
            $_SESSION['_flash'] = ['type'=>'success','message'=>"Đã mở đăng ký. Hạn: ".date('d/m/Y H:i', strtotime($regEnd)).". Đã thông báo ".count($svList)." sinh viên."];
        }
        $stmt->close();
        header('Location: semesters.php'); exit();
    }
    ...
    if ($action === 'close_registration') {
        ...
        $_SESSION['_flash'] = ['type'=>'warning','message'=>'Đã đóng đăng ký học phần.'];
        header('Location: semesters.php'); exit();
    }
}
...
$statusLabels = [
    'upcoming' => ['secondary','Sắp tới'],
    'open'     => ['success', 'Mở đăng ký'],
    'active'   => ['primary', 'Đang học'],
    'closed'   => ['dark',    'Đã kết thúc'],
];
...
<div class="admin-topbar">
    <div class="d-flex align-items-center gap-3">
        ...
        <span class="admin-topbar-title"><i class="bi bi-calendar3 me-2 text-navy"></i>Quản lý Học kỳ</span>
    </div>
...
<div class="card">
    <div class="card-header"><i class="bi bi-calendar3 me-2"></i>Danh sách Học kỳ</div>
    <div class="table-responsive">
        <table class="table table-hover mb-0">
            <thead class="table-light">
                <tr>
                    <th>Học kỳ</th><th>Năm học</th><th>Thời gian học</th>
                    <th>Đề xuất / duyệt</th><th>Đăng ký môn</th><th>Hạn nộp điểm</th>
                    <th class="text-center">Lớp HP</th><th class="text-center">Lượt ĐK</th>
            ...
            <tr>
                ...
                <td class="small text-muted">
                    <div>ĐX: <?php echo $sm['proposal_start'] ? date('d/m H:i', strtotime($sm['proposal_start'])) : '—'; ?>
                        - <?php echo $sm['proposal_end'] ? date('d/m H:i', strtotime($sm['proposal_end'])) : '—'; ?></div>
                    <div>Duyệt: <?php echo $sm['approval_start'] ? date('d/m H:i', strtotime($sm['approval_start'])) : '—'; ?>
                        - <?php echo $sm['approval_end'] ? date('d/m H:i', strtotime($sm['approval_end'])) : '—'; ?></div>
                </td>
                <td class="small">
                    <?php if ($isRegOpen): ?>
                    <span class="badge bg-success">Đang mở</span>
                    <br><small class="text-muted">Đến <?php echo date('d/m/Y H:i', strtotime($sm['register_end'])); ?></small>
                ...
                <td class="text-center">
                    ...
                    <button class="btn btn-xs btn-success" style="font-size:.7rem;padding:2px 6px"
                        ...
                        <i class="bi bi-unlock"></i> Mở ĐK
                    </button>
                    <?php elseif ($isRegOpen): ?>
                    <form method="post" class="d-inline" onsubmit="return confirm('Đóng đăng ký?')">
                        <?php echo csrfField(); ?>
...
<div class="modal fade" id="addSemModal" tabindex="-1">
    <div class="modal-dialog modal-lg"><div class="modal-content">
        <div class="modal-header"><h5 class="modal-title">Thêm Học kỳ mới</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
        <form method="post" action="semesters.php">
            <?php echo csrfField(); ?>
            <input type="hidden" name="action" value="add">
            <div class="modal-body"><div class="row g-3">
                <div class="col-md-6"><label class="form-label fw-semibold">Tên học kỳ <span class="text-danger">*</span></label>
                    <input type="text" name="name" class="form-control" required placeholder="VD: Học kỳ 1"></div>
                <div class="col-md-6"><label class="form-label fw-semibold">Năm học <span class="text-danger">*</span></label>
                    <input type="text" name="school_year" class="form-control" required placeholder="VD: 2025-2026"></div>
                <div class="col-md-6"><label class="form-label fw-semibold">Chế độ dữ liệu</label>
                    <select name="data_mode" class="form-select"><option value="system">Thật / Hệ thống</option><option value="test">Test / Demo</option></select></div>
                <div class="col-md-6"><label class="form-label fw-semibold">Ngày bắt đầu học</label>
                    <input type="date" name="start_date" class="form-control"></div>
                <div class="col-md-6"><label class="form-label fw-semibold">Ngày kết thúc học</label>
                    <input type="date" name="end_date" class="form-control"></div>
                <div class="col-md-6"><label class="form-label fw-semibold">Bắt đầu Khoa đề xuất</label>
                    <input type="datetime-local" name="proposal_start" class="form-control"></div>
                <div class="col-md-6"><label class="form-label fw-semibold">Kết thúc Khoa đề xuất</label>
                    <input type="datetime-local" name="proposal_end" class="form-control"></div>
                <div class="col-md-6"><label class="form-label fw-semibold">Bắt đầu Phòng ĐT duyệt</label>
                    <input type="datetime-local" name="approval_start" class="form-control"></div>
                <div class="col-md-6"><label class="form-label fw-semibold">Kết thúc Phòng ĐT duyệt</label>
                    <input type="datetime-local" name="approval_end" class="form-control"></div>
                <input type="hidden" name="proposal_deadline">
                <div class="col-md-6"><label class="form-label fw-semibold">Hạn nộp điểm (GV)</label>
...
<div class="modal fade" id="editSemModal" tabindex="-1">
    <div class="modal-dialog modal-lg"><div class="modal-content">
        <div class="modal-header"><h5 class="modal-title">Chỉnh sửa Học kỳ</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
        <form method="post" action="semesters.php">
            <?php echo csrfField(); ?>
            <input type="hidden" name="action" value="edit">
            <input type="hidden" name="id" id="editSemId">
            <div class="modal-body"><div class="row g-3">
                <div class="col-md-6"><label class="form-label fw-semibold">Tên học kỳ</label>
                    <input type="text" name="name" id="editSemName" class="form-control" required></div>
                <div class="col-md-6"><label class="form-label fw-semibold">Năm học</label>
                    <input type="text" name="school_year" id="editSemYear" class="form-control" required></div>
                <div class="col-md-6"><label class="form-label fw-semibold">Chế độ dữ liệu</label>
                    <select name="data_mode" id="editSemMode" class="form-select"><option value="system">Thật / Hệ thống</option><option value="test">Test / Demo</option></select></div>
                ...
                <div class="col-md-6"><label class="form-label fw-semibold">Ngày kết thúc</label>
                ...
                <div class="col-md-6"><label class="form-label fw-semibold">Bắt đầu đăng ký</label>
                    <input type="datetime-local" name="register_start" id="editRegStart" class="form-control"></div>
                <div class="col-md-6"><label class="form-label fw-semibold">Kết thúc đăng ký</label>
...
<div class="modal fade" id="openRegModal" tabindex="-1">
    <div class="modal-dialog"><div class="modal-content">
        <div class="modal-header bg-success text-white"><h5 class="modal-title">Mở Đăng ký Học phần</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
        <form method="post" action="semesters.php">
            <?php echo csrfField(); ?>
            <input type="hidden" name="action" value="open_registration">
            <input type="hidden" name="id" id="openRegSemId">
            <div class="modal-body">
                <p>Học kỳ: <strong id="openRegSemName"></strong></p>
                <label class="form-label fw-semibold">Số ngày mở đăng ký</label>
                <input type="number" name="reg_days" class="form-control" value="14" min="1" max="60">
                <div class="form-text">Hệ thống sẽ tự động thông báo đến tất cả sinh viên.</div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Hủy</button>
                <button type="submit" class="btn btn-success"><i class="bi bi-unlock me-1"></i>Mở đăng ký</button>
            </div>
        </form>
    </div></div>
</div>
