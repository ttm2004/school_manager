...
<head>
    ...
    <style>
        :root{--navy:#0d2d6b;--navy-dark:#071a45;--navy-light:#1a4fa0;--gold:#f5a623;--gold-dark:#d4891a;--sidebar-w:260px;--sidebar-collapsed-w:60px;}
        body{font-family:'Segoe UI',system-ui,sans-serif;background:#f0f2f7;color:#1a2340;}
        .adm-sidebar{position:fixed;top:0;left:0;width:var(--sidebar-w);height:100vh;background:linear-gradient(180deg,var(--navy-dark) 0%,var(--navy) 100%);overflow-y:auto;overflow-x:hidden;z-index:1000;transition:width .25s cubic-bezier(.4,0,.2,1);display:flex;flex-direction:column;}
        .adm-sidebar.collapsed{width:var(--sidebar-collapsed-w);}
        .adm-sidebar::-webkit-scrollbar{width:3px}.adm-sidebar::-webkit-scrollbar-thumb{background:rgba(255,255,255,.2);border-radius:3px}
        ...
        .sidebar-collapse-btn{position:absolute;right:-12px;top:50%;transform:translateY(-50%);width:24px;height:24px;border-radius:50%;background:#fff;border:2px solid #e2e8f0;display:flex;align-items:center;justify-content:center;cursor:pointer;z-index:10;box-shadow:0 2px 8px rgba(0,0,0,.15);transition:all .2s;color:var(--navy);font-size:.7rem;}
        ...
        .adm-sidebar.collapsed .sidebar-link .link-text,.adm-sidebar.collapsed .sidebar-link .bc{opacity:0;width:0;pointer-events:none;}
        ...
        .adm-sidebar.collapsed .sidebar-link::after{content:attr(data-tooltip);position:absolute;left:calc(var(--sidebar-collapsed-w) + 8px);top:50%;transform:translateY(-50%);background:rgba(13,45,107,.95);color:#fff;padding:5px 10px;border-radius:6px;font-size:.78rem;white-space:nowrap;pointer-events:none;opacity:0;transition:opacity .15s;z-index:9999;}
        ...
        .btn-navy{background:var(--navy);color:#fff;border:none;font-weight:600}.btn-navy:hover{background:var(--navy-light);color:#fff}
        .btn-gold{background:var(--gold);color:var(--navy-dark);border:none;font-weight:700}.btn-gold:hover{background:var(--gold-dark);color:var(--navy-dark)}
        .modal-header{background:var(--navy);color:#fff;border-bottom:3px solid var(--gold)}.modal-header .btn-close{filter:invert(1)}
        @media(max-width:991px){.adm-sidebar{transform:translateX(-100%)}.adm-sidebar.show{transform:translateX(0)}.adm-main{margin-left:0}.sb-toggle{display:flex!important}}
        ...
        .sb-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,.4);z-index:999}.sb-overlay.show{display:block}
    </style>
</head>
<body>
...
<div class="adm-sidebar" id="admSidebar">
    <div class="sidebar-brand">
        <div class="sidebar-brand-icon"><i class="bi bi-cash-coin"></i></div>
        <div class="sidebar-brand-text">
            <div class="t">Phòng Kế toán</div>
            <div class="s">TDMU Finance</div>
        </div>
    ...
    <nav class="sidebar-nav">
        ...
        <a href="/university/finance/index.php" class="sidebar-link <?php echo $_currentPage==='index.php'?'active':''; ?>" data-tooltip="Dashboard">
            <i class="bi bi-speedometer2"></i><span class="link-text"> Dashboard</span>
        </a>

        <?php if ($_isManager): ?>
        <div class="sidebar-sec">Quản lý</div>
        <a href="/university/finance/periods.php" class="sidebar-link <?php echo $_currentPage==='periods.php'?'active':''; ?>" data-tooltip="Đợt thu học phí">
            <i class="bi bi-calendar-range-fill"></i><span class="link-text"> Đợt thu học phí</span>
        </a>
        <?php endif; ?>

        <div class="sidebar-sec">Hóa đơn</div>
        <a href="/university/finance/invoices.php" class="sidebar-link <?php echo $_currentPage==='invoices.php'?'active':''; ?>" data-tooltip="Danh sách hóa đơn">
            <i class="bi bi-receipt"></i><span class="link-text"> Danh sách hóa đơn</span>
            <?php if ($_pendingCount > 0): ?><span class="bc"><?php echo $_pendingCount; ?></span><?php endif; ?>
        </a>
        <a href="/university/finance/payments.php" class="sidebar-link <?php echo $_currentPage==='payments.php'?'active':''; ?>" data-tooltip="Lịch sử thanh toán">
            <i class="bi bi-clock-history"></i><span class="link-text"> Lịch sử thanh toán</span>
        </a>

        <?php if ($_isManager): ?>
        <div class="sidebar-sec">Báo cáo</div>
        <a href="/university/finance/reports.php" class="sidebar-link <?php echo $_currentPage==='reports.php'?'active':''; ?>" data-tooltip="Báo cáo">
        ...
        <?php endif; ?>
        <a href="/university/staff/profile.php" class="sidebar-link" data-tooltip="Thông tin cá nhân">
...
(function(){
    const K='tdmu_fin_sidebar';
    const s=document.getElementById('admSidebar'),m=document.getElementById('admMain'),b=document.getElementById('sidebarCollapseBtn');
    function set(c){c?(s.classList.add('collapsed'),m.classList.add('collapsed')):(s.classList.remove('collapsed'),m.classList.remove('collapsed'));try{localStorage.setItem(K,c?'1':'0');}catch(e){}}
    try{if(localStorage.getItem(K)==='1')set(true);}catch(e){}
    if(b)b.addEventListener('click',function(e){e.stopPropagation();set(!s.classList.contains('collapsed'));});
})();
</script>
